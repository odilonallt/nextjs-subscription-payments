          ...
          <div className="grid gap-8">
            ...
            <div className="grid gap-6 sm:grid-cols-2">
              <div>
                ...
                <input type="number" onChange={(e) => setQuantity(Number(e.target.value))} className="mt-2 w-full rounded-2xl bg-slate-50 p-4 text-lg font-bold outline-none" placeholder="0" />
              </div>
              <div>
                <label className="text-sm font-bold text-slate-400 uppercase">Niveau d'exigence</label>
                <select onChange={(e) => setComplexity(Number(e.target.value))} className="mt-2 w-full rounded-2xl bg-slate-50 p-4 text-lg font-bold outline-none">
            ...
            <button onClick={handleEstimate} className="flex items-center justify-center rounded-2xl bg-indigo-600 py-5 font-black text-white hover:bg-indigo-700 transition-all">
              GÉNÉRER L'EXPERTISE
            </button>
          </div>
        ...
        {user.compteur_essais >= 2 && !user.est_premium && (
          <div className="mt-8 rounded-3xl border-2 border-indigo-600 bg-white p-8 text-center shadow-xl">
            ...
            <div className="mt-6 flex flex-col items-center gap-4">
              <a href="https://buy.stripe.com/9B614g1Ri9sweUY9pWbfO04" target="_blank" className="w-full max-w-xs rounded-2xl bg-indigo-600 py-4 font-black text-white shadow-lg">OBTENIR MON CODE VIP</a>
              <div className="flex w-full max-w-md items-center gap-2">
                <input type="text" placeholder="CODE VIP" value={licenseCode} onChange={(e) => setLicenseCode(e.target.value)} className="flex-1 rounded-xl border p-4 uppercase outline-none" />
                <button onClick={handleActivate} className="rounded-xl bg-slate-900 px-6 py-4 font-bold text-white">ACTIVER</button>
              </div>
